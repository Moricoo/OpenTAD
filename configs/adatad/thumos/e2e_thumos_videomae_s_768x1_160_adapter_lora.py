...
solver = dict(
    ...
    test=dict(batch_size=1, num_workers=2),  # 测试时减少batch_size和workers以避免OOM
    clip_grad_norm=1,
    amp=True,
    fp16_compress=True,
    static_graph=True,
...
optimizer = dict(
    ...
    backbone=dict(
        lr=0,
        weight_decay=0,
        custom=[dict(name="adapter", lr=2e-4, weight_decay=0.05), dict(name="lora", lr=2e-4, weight_decay=0.05)],
        exclude=["backbone"],
    ),
)
...
work_dir = "exps/thumos/adatad/e2e_actionformer_videomae_s_768x1_160_adapter_lora"
