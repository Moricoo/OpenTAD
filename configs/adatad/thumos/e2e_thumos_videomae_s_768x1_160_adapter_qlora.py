...
model = dict(
    backbone=dict(
        ...
        backbone=dict(
            ...
            adapter_index=list(range(12)),  # 所有层都使用QLoRA Adapter
            # QLoRA配置
            lora_r=16,  # LoRA rank
            lora_alpha=32,  # LoRA alpha (缩放因子)
            lora_dropout=0.1,  # LoRA dropout
            quantize_bits=4,  # 4-bit quantization
            use_plain_adapter=False,  # 使用完整版Adapter（包含temporal convolution）
        ),
...
work_dir = "exps/thumos/adatad/e2e_actionformer_videomae_s_768x1_160_adapter_qlora"
